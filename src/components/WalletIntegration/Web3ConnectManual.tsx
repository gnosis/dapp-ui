...
// Types
import { Web3Api, provider as ProviderType } from '../../types'
...
const ManualConnector: React.FC = () => {
    const [web3, setWeb3] = useState<Web3Api>()

    useEffect(() => {
        web3Connect.on('connect', async (provider: ProviderType) => {
            try {
                console.log(
    ...
    return (
        <>
            <Button onClick={() => web3Connect.toggleModal()} secondary>
                Open web3connect Modal
            </Button>
            {web3 && <DisplayWeb3 title="WEB3CONNECT MANUAL" web3Api={web3} />}
        </>
    )
}
